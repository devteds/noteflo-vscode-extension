...
exports.InvoiceGenerator = exports.getConfiguredTimezone = exports.formatDateInTimezone = void 0;
const vscode = require("vscode");
const fs = require("fs");
const path = require("path");
const jspdf_1 = require("jspdf");
// Utility function to format dates in configured timezone
function formatDateInTimezone(date = new Date(), timezone = 'America/Chicago', format = 'iso') {
    switch (format) {
        case 'iso':
...
function getConfiguredTimezone(workspaceRoot) {
...
class InvoiceGenerator {
    ...
    async generateInvoice() {
        ...
        vscode.window.showInformationMessage(`💰 Generated invoice: ${invoiceNumber} (${invoiceData.totalHours.toFixed(1)}h @ ${invoiceData.currency}${invoiceData.hourlyRate}/h = ${invoiceData.currency}${invoiceData.total.toFixed(2)}) - Both MD & PDF created!`);
    }
    generateInvoiceNumber(period) {
    ...
    calculateInvoiceData(period, timeEntries, config) {
    ...
    async generateMarkdownInvoice(invoiceData, config) {
        const timezone = getConfiguredTimezone(this.workspaceRoot);
...
`;
    ...
    async generatePDFInvoice(invoiceData, config) {
        const timezone = getConfiguredTimezone(this.workspaceRoot);
        ...
        () => {
            ...
            if (config.business.address) {
                config.business.address.split('\n').forEach(line => {
                    if (line.trim())
                        layout.addText(line.trim(), 10);
                });
            }
            if (config.business.email)
                layout.addText(`Email: ${config.business.email}`, 10);
            if (config.business.phone)
                layout.addText(`Phone: ${config.business.phone}`, 10);
        }, 
        // Right column (TO)
        () => {
            layout.addSubheading('TO:');
            layout.addText(config.client.name, 10, 'bold');
            if (config.client.contact)
                layout.addText(`Attn: ${config.client.contact}`, 10);
            ...
            if (config.client.email)
                layout.addText(`Email: ${config.client.email}`, 10);
        });
        layout.addLine();
        // Time Entries Table
        layout.addSubheading('TIME ENTRIES:');
        layout.addTable(['Date', 'Description', 'Hours'], [30, 100, 25], invoiceData.timeEntries.map(entry => [
    ...
    async viewInvoices() {
        ...
        const selectedInvoice = await vscode.window.showQuickPick(invoices.map(f => f.replace('.md', '')), { placeHolder: 'Select invoice to view' });
        if (!selectedInvoice)
            return;
...
class PDFLayout {
    ...
    addSubheading(text, fontSize = 12) {
    ...
    addTwoColumns(leftColumn, rightColumn) {
    ...
    addTable(headers, columnWidths, rows) {
    ...
    addWrappedText(text, fontSize = 10) {
        this.doc.setFontSize(fontSize);
        this.doc.setFont('helvetica', 'normal');
    ...
    calculateColumnPositions(widths) {
